...
@Component({
  selector: 'app-inazumago-dos',
  templateUrl: './inazumago-dos.component.html',
  styleUrls: ['./inazumago-dos.component.css']
})
export class InazumagoDosComponent {
  playlistId = 'PLSXT4sOipmuzq2rGigtuXQgHt7Dic0A1v';
  videos: any[] = [];
  ...
  fetchPlaylistVideos(): void {
    ...
    const getVideos = () => {
      const url = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${this.playlistId}&maxResults=51&pageToken=${nextPageToken}&key=${apiKey}`;
      ...
      this.http.get(url).subscribe((response: any) => {
        const videosFromResponse = response.items.map((item: any) => {
          ...
          const title = snippet.title.replace('Inazuma Eleven Go Chrono Stones -', ''); // Remove "Inazuma Eleven Castellano"
